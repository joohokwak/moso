window.addEventListener('DOMContentLoaded', function () {
  ...
  const images = document.querySelectorAll(
    '#shopping_buy .goods_img .swiper-slide'
  );
  if (images) {
    images.forEach((img) => {
      img.addEventListener('click', function () {
        document.querySelector('#shopping_buy .main_img img').src =
          img.children[0].src;
      });
    });
  }
  ...
  const sizes = document.querySelectorAll(
    '#shopping_buy .select_detail .size a'
  );
  const delivery = document.querySelectorAll(
    '#shopping_buy .select_detail .delivery a'
  );
  const priceEm = document.querySelector(
    '#shopping_buy .goods_select .total_price em'
  );
    ...
    delivery.forEach((item, idx) => {
      item.addEventListener('click', function (e) {
        ...
        if (delivery[1].classList.contains('active')) {
          priceEm.innerText = (Number(price) * addPr + 16000)
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        } else if (delivery[2].classList.contains('active')) {
          priceEm.innerText = (Number(price) * addPr + 30000)
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        } else if (delivery[3].classList.contains('active')) {
          priceEm.innerText = (Number(price) * addPr + 46000)
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        } else {
          priceEm.innerText = (Number(price) * addPr)
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        }
      });
            ...
            .then((data) => {
              if (data != 0) {
                if (like.indexOf('off') !== -1) {
                  this.children[0].src = like.replace(
                    'wish_off.png',
                    'wish_on.png'
                  );
                } else {
  ...
  const reviewView = document.querySelectorAll(
    '#view_review .board_body .display_view'
  );
  const reviewNone = document.querySelector(
    '#view_review .board_body .display_view .display'
  );
  ...
  const showDetail = document.querySelector(
    '#view_info .detail_more_box .table i'
  );
  const detailBox = document.querySelector('#view_info .detail_more_box');
  const detailImage = document.querySelector(
    '#view_info .detail_more_box .more_size'
  );
  if (detailBox) {
